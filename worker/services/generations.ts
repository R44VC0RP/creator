...
function requireReplicateToken(env: Env) {
  if (!env.REPLICATE_API_TOKEN) {
    throw new ApiError(
      ...
      "Replicate is not configured on this server."
    )
  }
...
function requireWaveSpeedToken(env: Env) {
  if (!env.WAVESPEED_API_KEY) {
    throw new ApiError(
      503,
      "WAVESPEED_NOT_CONFIGURED",
      "WaveSpeed is not configured on this server."
    )
  }
...
async function replicateRequest(env: Env, path: string, init?: RequestInit) {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      ...
      detail || "Replicate request failed."
    )
  }
  return response
}
...
async function uploadInputFile(
  ...
  const response = await replicateRequest(env, "/files", {
    method: "POST",
    body: form,
  })
...
async function cancelPrediction(env: Env, predictionId: string) {
  const response = await replicateRequest(
    env,
    `/predictions/${encodeURIComponent(predictionId)}/cancel`,
    { method: "POST" }
  )
  return response.json() as Promise<Prediction>
}
...
async function waveSpeedRequest(env: Env, path: string, init?: RequestInit) {
  ...
  if (!response.ok) {
    ...
    throw new ApiError(
      502,
      "WAVESPEED_REQUEST_FAILED",
      detail || "WaveSpeed request failed."
    )
  }
  ...
  if (!payload.data)
    throw new ApiError(
      502,
      "WAVESPEED_REQUEST_FAILED",
      payload.message || "WaveSpeed response was invalid."
    )
  return payload.data
}
...
async function createWaveSpeedSeedancePrediction(env: Env, turn: TurnRow) {
  ...
  if (!input)
    throw new ApiError(
      ...
      "Seedance requires a source image."
    )
...
async function createWaveSpeedGptPrediction(env: Env, turn: TurnRow) {
  ...
  for (const input of storedInputs) {
    if (input.deleted_at || !input.r2_key) {
      throw new ApiError(
        409,
        "REFERENCE_ASSET_MISSING",
        "A reference image for this generation is no longer available."
      )
    }
    inputUrls.push(
  ...
  return waveSpeedRequest(env, endpoint, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({
      prompt:
        inputUrls.length > 0 ? turn.compiled_prompt : turn.authored_prompt,
      ...(inputUrls.length > 0 ? { images: inputUrls } : {}),
      aspect_ratio: turn.aspect_ratio,
      resolution:
        turn.quality === "low" ? "1k" : turn.quality === "medium" ? "2k" : "4k",
...
export async function findTurn(env: Env, turnId: string) {
  const turn = await env.DB.prepare("SELECT * FROM turns WHERE id = ?")
    .bind(turnId)
    .first<TurnRow>()
  if (!turn) {
...
export async function createPrediction(
  ...
  if (turn.provider === "wavespeed") {
    ...
    const errorMessage =
      prediction.status === "failed"
        ? friendlyPredictionError(prediction.error, turn.generation_mode)
        : null
    await env.DB.prepare(
      "UPDATE turns SET replicate_prediction_id = ?, status = ?, error_message = ?, completed_at = ? WHERE id = ?"
    )
      .bind(
        prediction.id,
        status,
        errorMessage,
        prediction.status === "failed" ? now() : null,
        turn.id
      )
      .run()
    await recordGenerationEvent(
      env,
      turn.id,
      prediction.status === "failed" ? "failed" : "status",
  ...
  for (const input of storedInputs) {
    ...
    const uploaded = await uploadInputFile(env, blob, {
      turnId: turn.id,
      role: input.role,
    })
  ...
  if (turn.model === GROK_IMAGE_MODEL && inputUrls.length > 1) {
    throw new ApiError(
      ...
      "Grok Imagine Quality accepts only one image input."
    )
  }

  const input =
    turn.model === GROK_IMAGE_MODEL
      ? {
          prompt: turn.authored_prompt,
      ...
      : turn.generation_mode === "video" &&
          turn.model === LEGACY_REPLICATE_SEEDANCE_MODEL
        ? {
            prompt: turn.authored_prompt,
            image: inputUrls[0],
            duration: turn.video_duration ?? 5,
  ...
  await env.DB.prepare(
    "UPDATE turns SET replicate_prediction_id = ?, status = ? WHERE id = ?"
  )
...
export async function reconcileTurn(
  ...
  if (turn.provider === "wavespeed") {
    const prediction = await waveSpeedRequest(
      env,
      `/predictions/${encodeURIComponent(turn.replicate_prediction_id)}/result`
    )
    if (prediction.status === "failed") {
      const message = friendlyPredictionError(
        prediction.error,
        turn.generation_mode
      )
      await env.DB.prepare(
        "UPDATE turns SET status = 'failed', error_message = ?, completed_at = ? WHERE id = ? AND status NOT IN ('succeeded', 'failed', 'canceled')"
      )
        .bind(message, now(), turn.id)
        .run()
      return findTurn(env, turn.id)
    }
    if (prediction.status !== "completed") return turn
    const claimed = await env.DB.prepare(
      "UPDATE turns SET status = 'persisting' WHERE id = ? AND status NOT IN ('succeeded', 'failed', 'canceled')"
    )
      .bind(turn.id)
      .run()
    ...
    if (!outputUrl)
      throw new ApiError(
        ...
        `WaveSpeed did not return an ${turn.generation_mode} output URL.`
      )
    const assetId =
      turn.generation_mode === "video"
        ? await storeGeneratedVideo(
            ...
            (await loadTurnInputs(env, turn.id))[0].asset_id
          )
        : await storeGeneratedOutput(
            env,
            turn.id,
            turn.conversation_id,
            await fetch(outputUrl),
    ...
    await env.DB.prepare(
      "UPDATE turns SET status = 'succeeded', output_asset_id = ?, completed_at = ?, error_message = NULL WHERE id = ?"
    )
      .bind(assetId, now(), turn.id)
      .run()
    await recordGenerationEvent(env, turn.id, "completed", {
      status: "succeeded",
      assetId,
    })
    return findTurn(env, turn.id)
  ...
  const prediction =
    suppliedPrediction ??
    (await getPrediction(env, turn.replicate_prediction_id))
  if (prediction.id !== turn.replicate_prediction_id) {
    throw new ApiError(
      409,
      "PREDICTION_MISMATCH",
      "Replicate prediction does not match this generation."
    )
  }
  ...
  if (
    ...
    const status: TurnStatus =
      prediction.status === "failed" ? "failed" : "canceled"
    const message =
      prediction.status === "failed"
        ? friendlyPredictionError(prediction.error, turn.generation_mode)
        : null
    await env.DB.prepare(
      "UPDATE turns SET status = ?, error_message = ?, completed_at = ? WHERE id = ? AND status NOT IN ('succeeded', 'failed', 'canceled')"
    )
  ...
  if (prediction.status !== "succeeded") {
    ...
    if (status !== turn.status) {
      await env.DB.prepare(
        "UPDATE turns SET status = ? WHERE id = ? AND status NOT IN ('persisting', 'succeeded', 'failed', 'canceled')"
      )
        .bind(status, turn.id)
        .run()
  ...
  try {
    ...
    await env.DB.prepare(
      "UPDATE turns SET status = 'processing', error_message = ? WHERE id = ? AND status = 'persisting'"
    )
...
export async function cancelTurn(env: Env, turnId: string) {
  ...
  if (turn.replicate_prediction_id) {
    if (turn.provider === "replicate")
      await cancelPrediction(env, turn.replicate_prediction_id)
  }
  await env.DB.prepare(
    "UPDATE turns SET status = 'canceled', completed_at = ? WHERE id = ?"
  )
    .bind(now(), turnId)
    .run()
...
function friendlyPredictionError(
  error: unknown,
  mode: TurnRow["generation_mode"]
) {
  ...
  return mode === "video"
    ? "Video generation failed. Try a different motion prompt or source image."
    : "Image generation failed. Try changing the prompt or references."
}
...
function firstOutputUrl(output: unknown) {
  ...
  if (typeof candidate !== "string") {
    throw new ApiError(
      502,
      "INVALID_MODEL_OUTPUT",
      "Replicate did not return an image output URL."
    )
  }
  return candidate
}
