...
- Generation: Replicate `openai/gpt-image-2` and text-only `xai/grok-imagine-image-quality`.
...
Production generations provide Replicate with a completion webhook URL only when `PUBLIC_APP_URL` is HTTPS. Deliveries are signature-validated, idempotent, and used to persist completed outputs even when no browser SSE connection remains active.
